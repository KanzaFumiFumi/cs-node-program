...
BEAR_MAX_MS = 30000
# 速報の送信回数：取りこぼし対策として、同じ速報を100msごとにこの回数送る（GW の中継も同じ回数）。
BEAR_REPEAT = 3
...
LEN_BEAR_RELAY = struct.calcsize(FMT_BEAR_RELAY)
...
class Node:
    ...
    def on_recv(self, mac, msg):
        ...
        if n == LEN_BEACON:
            ...
            if v[0] != MAGIC or v[1] != T_BEAR or v[2] != sid or v[4] >= N or v[4] == sid:
                return
            if self.gw != ME:
                return
            self.queue_relay(v[2], v[3], v[4], time.ticks_add(time.ticks_ms(), -v[5]), v[6], v[7])
    ...
    def bear_report(self, j, t0, length, drop):
        ...
        if self.gw == ME:
            self.queue_relay(ME, self.rid, j, t0, length, drop)

    def queue_relay(self, a, rid, b, t0, length, drop):
        now = time.ticks_ms()
        self.relay_seen = [s for s in self.relay_seen if time.ticks_diff(now, s[2]) < BEAR_SEEN_MS]
        for s in self.relay_seen:
            if s[0] == a and s[1] == rid:
                return
        self.relay_seen.append((a, rid, now))
        self.relay_out.append([a, rid, b, t0, length, drop, BEAR_REPEAT])

    def send_bears(self):
        if not self.bear_out and not self.relay_out:
            return
        now = time.ticks_ms()
        for it in self.bear_out:
            ...
            it[6] -= 1
        for it in self.relay_out:
            age = max(0, min(time.ticks_diff(now, it[3]), 0xFFFF))
            self.send(struct.pack(FMT_BEAR_RELAY, MAGIC, T_BEAR_RELAY, ME, it[0], it[1], it[2], age, it[4], it[5]))
            it[6] -= 1
        self.bear_out = [it for it in self.bear_out if it[6] > 0]
        self.relay_out = [it for it in self.relay_out if it[6] > 0]
