    ...
    def on_recv(self, mac, msg):
        ...
        if n == LEN_BEACON:
            ...
            if self.measuring or self.holding:
                return
            print(' '.join(['%s:%d' % (COLS[i], vals[i]) for i in range(N * M)]) + self.bear_text())
        elif n == LEN_BEAR_RELAY:
            v = struct.unpack(FMT_BEAR_RELAY, msg)
            if v[0] != MAGIC or v[1] != T_BEAR_RELAY or v[2] != sid:
                return
            self.on_bear(v[3], v[5], v[6], v[7], v[8])
